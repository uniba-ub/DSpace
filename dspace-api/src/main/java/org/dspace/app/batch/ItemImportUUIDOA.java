 ...
 */
public class ItemImportUUIDOA {

    private DSpace dspace = new DSpace();

    /** logger */
    private static Logger log = Logger.getLogger(ItemImportUUIDOA.class);
    ...
    public static void main(String[] argv) {
        ...
        try {
            context = new Context();
            new ItemImportUUIDOA().impRecord(context, argv);
            context.complete();
        } catch (Exception e) {
    ...
    public UUID impRecord(Context context, String[] argv) throws Exception {
        // instantiate loader
        ItemImportUUIDOA myLoader = new ItemImportUUIDOA();
        ...
        options.addOption("u", "uuid", true, "add with predefined uuid of item");
        ...
        if (line.hasOption('u')) {
        	predefineduuid = UUID.fromString(line.getOptionValue('u').trim());
        }
        ...
        try {
            ...
            if (command.equals("add")) {
                item_id = myLoader.addItem(context, mycollections, impRecord, handle, clearOldBitstream, predefineduuid);
    ...
    private UUID addItem(Context c, Collection[] mycollections, ImpRecord impRecord, String handle,
    	...
    	return addItem(c, mycollections, impRecord, handle, clearOldBitstream, null);
    }
     ...
     */
    private UUID addItem(Context c, Collection[] mycollections, ImpRecord impRecord, String handle,
            boolean clearOldBitstream, UUID predefineduuid) throws Exception {
        ...
        //uniba: set old uuid as predefined for creating the workspaceitem
