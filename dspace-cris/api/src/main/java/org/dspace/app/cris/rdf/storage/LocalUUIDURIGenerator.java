...
import org.dspace.app.cris.rdf.storage.LocalUUIDURIGenerator;
 ...
 *         
 *         Extension for LocalURIGenerator. Using uuid as Identifier for
 *         Cris-Objects.
 * @commentor Florian Gantner (florian.gantner@uni-bamberg.de)
 */
public class LocalUUIDURIGenerator implements URIGenerator {
	private static final Logger log = Logger.getLogger(LocalUUIDURIGenerator.class);
	...
	public String generateIdentifier(Context context, DSpaceObject dso) throws SQLException {
		...
		if (dso instanceof ACrisObject) {
			...
			sb.append("/");
			sb.append(addpath(dso, aco.getUuid()));
			return sb.toString();
		}
		...
		// for dso: use path and handle as parameter
